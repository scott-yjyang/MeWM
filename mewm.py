...
def get_gpt4o_drug_embolism_sets(prompt, api_key):
    openai.api_key = api_key
    response = openai.ChatCompletion.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a medical AI assistant, responsible for generating a list of drugs and embolisms for TACE treatment."},
    ...
    return result['drug_set'], result['embolism_set']


def get_gpt4o_drug_embolism_sets_with_image(prompt, api_key, image_bytes):
    openai.api_key = api_key
    response = openai.ChatCompletion.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a medical AI assistant, responsible for generating a list of drugs and embolisms for TACE treatment based on the provided CT image."},
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": prompt},
                    {"type": "image_url", "image_url": "data:image/png;base64," + base64.b64encode(image_bytes.getvalue()).decode()}
                ]
            }
        ],
        temperature=0.2,
...
if __name__ == "__main__":
    ...
    drug_set, embolism_set = get_gpt4o_drug_embolism_sets(prompt, api_key)
    print("GPT-4o generated drug_set:", drug_set)
    print("GPT-4o generated embolism_set:", embolism_set)
    ...
    for i, data_dict in tqdm(enumerate(dataloader)):
        ...
        img_bytes = ct_tensor_to_png_bytes(x.cpu().squeeze(1))  # [B, 1, D, H, W] -> [D, H, W]
        drug_set, embolism_set = get_gpt4o_drug_embolism_sets_with_image(prompt, api_key, img_bytes)
        print("GPT-4o generated drug_set:", drug_set)
        print("GPT-4o generated embolism_set:", embolism_set)
