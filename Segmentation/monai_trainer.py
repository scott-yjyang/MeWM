        ...
        for bs in range(data.shape[0]):
            ...
            if 'kidney_label' in data_name or 'liver_label' in data_name or 'pancreas_label' in data_name:
                if random.random() > sample_thresh:
                    ...
                    tumor_types = ['medium', 'large']
                    tumor_probs = np.array([0.8, 0.1, 0.1])
                    synthetic_tumor_type = np.random.choice(tumor_types, p=tumor_probs.ravel())
                    
                    # 保存生成前的图像和mask
                    vis_dir = os.path.join(args.logdir, 'visualization', f'epoch_{epoch}', f'sample_{idx}_{bs}')
                    os.makedirs(vis_dir, exist_ok=True)
                    
                    save_nifti(healthy_data, os.path.join(vis_dir, 'original_ct.nii.gz'))
                    save_nifti(healthy_target, os.path.join(vis_dir, 'original_mask.nii.gz'))
                    ...
                    # 保存生成后的图像和mask
                    save_nifti(synt_data, os.path.join(vis_dir, f'synthetic_{synthetic_tumor_type}_ct.nii.gz'))
                    save_nifti(synt_target, os.path.join(vis_dir, f'synthetic_{synthetic_tumor_type}_mask.nii.gz'))
                    
                    # 保存生成信息到文本文件
                    with open(os.path.join(vis_dir, 'generation_info.txt'), 'w') as f:
                        f.write(f'Original image: {data_name}\n')
                        f.write(f'Tumor type: {synthetic_tumor_type}\n')
                        f.write(f'Organ type: {args.organ_type}\n')
