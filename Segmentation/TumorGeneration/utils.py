...
def random_select(mask_scan, organ_type):
    ...
    if np.sum(mask_scan) == 0:
        raise ValueError("Empty mask: no organ region found in the mask")
        
    # 获取器官区域的范围
    z_indices = np.where(np.any(mask_scan, axis=(0, 1)))[0]
    if len(z_indices) == 0:
        raise ValueError("No valid z-axis slices found in the mask")
        
    z_start, z_end = z_indices.min(), z_indices.max()
    ...
    while 1:
        ...
        flag+=1
        if flag > 50:  # 添加最大尝试次数
            raise ValueError("Could not find valid location after 50 attempts")

    coordinates = np.argwhere(liver_mask == 1)
    if len(coordinates) == 0:
        raise ValueError("No valid coordinates found in the selected slice")
