...
def _get_transform(args):
    ...
    [
        ...
        transforms.ScaleIntensityRanged(
            keys=["image"], a_min=-175, a_max=600,
    ...
    val_transform = transforms.Compose(
        [
            ...
            transforms.ScaleIntensityRanged(keys=["image"], a_min=-175, a_max=600, b_min=-1.0, b_max=1.0, clip=True),
...
def main_worker(gpu, args):
    ...
    train_txt = os.path.join(datafold_dir, 'hcc_{}_train_{}.txt'.format(tumor_type, fold))
    ...
    for line in open(train_txt):
        ...
        train_img_real.append(data_root + line.strip().split('\t')[4])
        train_lbl_real.append(data_root + line.strip().split('\t')[5])
        train_name_real.append(name)
        # else:
    ...
    val_name=[]
    val_pseudo_lbl=[]
    for line in open(os.path.join(datafold_dir, 'hcc_{}_val_{}.txt'.format(tumor_type, fold))):
        name = line.strip().split('\t')[1].split('.')[0]
        # val_img.append(data_root + line.strip().split('\t')[0])
        # val_lbl.append(data_root + line.strip().split('\t')[1])
        # val_pseudo_lbl.append(data_root + line.strip().split('\t')[3])
        val_img.append(data_root + line.strip().split('\t')[4])
        val_lbl.append(data_root + line.strip().split('\t')[5])
        val_pseudo_lbl.append(data_root + line.strip().split('\t')[7])

        val_name.append(name)
    # print(val_pseudo_lbl[0])
